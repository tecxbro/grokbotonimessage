...
5. Claims serialize unresolved `queued`, `blocked`, and `unknown-outcome` work within one conversation, without making an unrelated conversation on the same line depend on it. `space.create` uses its explicit line-scoped creation dependency. Terminal predecessors do not block; fences still advance on new ownership and expired/stale claims are rejected.
