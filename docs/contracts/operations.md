...
Registration alone is not implementation. Production capability evaluation now requires an explicit matching implementation declaration, complete implementation, required configuration and operational dependencies. Partial/missing implementations remain unavailable; actual resource validity and authorization are checked separately. The generated inventory still needs final coordinator reconciliation after A-E. The local `media.import` method stages bytes and is not a 45th messaging action.

Installed spectrum-ts 12.8.0 exposes poll builders but no public narrowed poll management namespace or native incoming option IDs. Advanced-kit examples do not authorize private SDK access or another client. Worker B must preserve this documented compatibility boundary and report required upstream support. Worker C must preserve the absence of a real authenticated callback backend and universal update URL mapping instead of inventing a protocol.
