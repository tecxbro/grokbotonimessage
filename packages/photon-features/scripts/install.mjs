...
export async function installRelease({ archivePath, checksum, root }) {
  ...
  if (archive.metadata.completionContract === 1) for (const name of ['dist/src/host/text-producer.js', 'dist/src/host/card-backend.js',
    'dist/src/host/card-browser.js', 'dist/src/host/authority-admin.js', 'schemas/host-configuration-v2.json',
    'schemas/authority-transition-v1.json', 'schemas/stream.open.json', 'schemas/stream.append.json',
    'schemas/stream.close.json', 'schemas/stream.abort.json', 'examples/production-inventory.json',
    'scripts/generate-configuration.mjs', 'npm-shrinkwrap.json']) if (!names.has(name)) throw new Error('INCOMPLETE_RELEASE');
  await prepare(root);
