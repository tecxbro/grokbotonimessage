...
// Profiles are an upper bound, never a grant. The owner must enumerate enable.
...
export async function generateProfiles(check = false) {
  ...
  for (const [name, allowedOperations] of Object.entries(profiles)) {
    const content = JSON.stringify({ version: 1, profile: name, allowedOperations,
      defaultEnabledOperations: [], explicitOwnerSelectionRequired: true,
      upstreamReleaseBlockers: upstreamPollOperations,
      note: 'This is a selection profile, not an executable host configuration. Supply real owner inputs and explicit enable; generation always disables activation.' }, null, 2) + '\n';
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    if (args.length === 1 && ['--profiles', '--check'].includes(args[0])) console.log(JSON.stringify(await generateProfiles(args[0] === '--check')));
      ...
      const config = generateConfiguration(JSON.parse(await readFile(args[0], 'utf8')));
      await writeFile(args[1], JSON.stringify(config, null, 2) + '\n', { flag: 'wx', mode: 0o600 });
      console.log(JSON.stringify({ generated: args[1], activation: 'disabled', operations: config.task.permissions.length }));
    }
